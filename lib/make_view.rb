...
class MakeView
  ...
  def read_log(dir_name)
    if(File.exist?(@log_root + dir_name + "/contents.json"))
      File.open(@log_root + dir_name + "/contents.json", "r") do |f|
        @json_data = JSON.load(f,nil, {:symbolize_names => true})
      end
    else 
      ...
      File.open(@log_root + dir_name + "/contents.json", "w") do |f|
